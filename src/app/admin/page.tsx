...
export default async function AdminDashboardPage() {
  ...
  const activeProjects = projects.filter((project) => project.status === "公開中").length;
        ...
        <aside className="space-y-4">
          <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div className="border-b border-slate-100 px-5 py-4">
              <p className="text-sm font-semibold text-teal-700">FirstAI</p>
              <p className="mt-1 text-xs text-slate-500">管理メニュー</p>
            </div>
            <nav className="p-3 text-sm">
              <SideLink active href="/admin" label="カレンダー" />
              <SideLink href="/admin/bookings" label="予約一覧" />
              <SideLink href="/admin/members" label="メンバー" />
              <SideLink href="/admin/settings" label="自分の設定" />
        ...
        <div>
          <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex flex-col justify-between gap-5 md:flex-row md:items-end">
              <div>
                <p className="text-sm font-semibold text-teal-700">カレンダー</p>
                <h1 className="mt-2 text-3xl font-semibold tracking-tight">予約ページ一覧</h1>
                <p className="mt-3 text-sm text-slate-500">
                  お客さんに共有する予約ページを管理します。ページ表示、URLコピー、公開状態の確認をここから行えます。
                </p>
              </div>
              <Link
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-3">
              ...
              <Stat label="今後の予約" value={`${upcomingBookings}`} note="登録済み予約" />
            </div>
          </section>
